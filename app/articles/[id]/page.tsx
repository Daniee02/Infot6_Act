      ...
      <div className="container-narrow" style={{ paddingBottom: 80 }}>
        ...
        <article className="prose">
          {(article.content || '').split('\n').map((line: string, i: number) => {
            ...
            if (line.startsWith('- '))  return <li key={i} style={{ marginLeft: 24 }}>{line.slice(2)}</li>
        ...
        <div className="card" style={{ padding: '24px 28px', marginTop: 48, display: 'flex', gap: 16, alignItems: 'flex-start' }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>{authorName}</div>
            <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
              ML practitioner sharing knowledge with the community.
            </div>
          </div>
        </div>
